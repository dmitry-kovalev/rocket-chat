...
function processImgs(html) {
  ...
  while (start >= 0) {
    ...
    let src = img.substring(sStart + 5, sEnd);const extrasRegex = /<\/?(?:div|address|section|article|span).*?>/gim
      const stylesRegex = /<\/?(?:b|i|a|img|em|p|h\d?).*?>/gim
      ...
      class Script {
          process_incoming_request({request}) {
              ...
              let message_icon = content.emoji;
              let attachments = [{}];
              let text = '';
              let plain = '';
              let username = content.username || content.alias;
